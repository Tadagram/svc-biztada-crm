...
export default async function aiKnowledgeHandler(fastify: FastifyInstance) {
  ...
  fastify.delete('/:id', async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    try {
      const res = await fetch(`${AI_CONTROLLER_URL}/api/v1/strategy/knowledge/${request.params.id}`, {
        method: 'DELETE',
        headers: { authorization: request.headers.authorization || '' },
      });
      const data = await res.json();
  ...
  fastify.post('/query', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const res = await fetch(`${AI_CONTROLLER_URL}/api/v1/strategy/knowledge?category=${category || ''}`, {
        headers: { authorization: request.headers.authorization || '' },
      });
      const data = await res.json();
