...
export async function handler(
  ...
  return reply.send({
    ...
    data: packages.map((item: any) => {
      const bonusLicenseKeyCount = calcBonusLicenseCount(
        item.license_key_count,
        item.agent_discount_percent,
        item.type,
      );
      const totalLicenseKeyCount = item.license_key_count + bonusLicenseKeyCount;

      return {
        bonus_percent: item.agent_discount_percent,
        bonus_license_key_count: bonusLicenseKeyCount,
        total_license_key_count: totalLicenseKeyCount,
