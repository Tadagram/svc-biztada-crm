...
export async function handler(
  request: FastifyRequest<{ Body: ContentEngineBody }>,
  reply: FastifyReply,
) {
  ...
  await request.prisma.$executeRaw`
    UPDATE strategy_content_engine
    SET deleted_at = NOW()
  ...
  await request.prisma.$executeRaw`
    INSERT INTO strategy_content_engine (strategy_content_engine_id, business_id, user_id, guest_id, payload, is_demo, created_at, updated_at)
    VALUES (
      ${newId},
      ${effectiveBusinessId},
      ${effectiveUserId},
