...
interface UpsertContentNetworkBody {
  payload: unknown;
  guestId?: string;
  businessId?: string;
  userId?: string;
...
export async function handler(
  request: FastifyRequest<{ Body: UpsertContentNetworkBody }>,
  reply: FastifyReply,
) {
  ...
  // Guest path
  if (effectiveGuestId) {
    const existing = await request.prisma.$queryRaw<StrategyContentNetworkRow[]>`
      SELECT strategy_content_network_id FROM strategy_content_network
      WHERE deleted_at IS NULL AND guest_id = ${effectiveGuestId} LIMIT 1
    `;
    let created = false;
    let rowId: string;
    if (existing.length > 0) {
      rowId = existing[0].strategy_content_network_id;
      await request.prisma.$executeRaw`
        UPDATE strategy_content_network SET payload = ${payloadJson}, updated_at = NOW()
        WHERE strategy_content_network_id = ${rowId}
      `;
    } else {
      const newId = crypto.randomUUID();
      await request.prisma.$executeRaw`
        INSERT INTO strategy_content_network
          (strategy_content_network_id, business_id, user_id, guest_id, payload, is_demo, created_at, updated_at)
  ...
  // Legacy path
  let existing: StrategyContentNetworkRow[];
  if (effectiveUserId !== null) {
    existing = await request.prisma.$queryRaw<StrategyContentNetworkRow[]>`
      SELECT strategy_content_network_id FROM strategy_content_network
