...
interface UpsertContentPlanBody {
  payload: unknown;
  guestId?: string;
  businessId?: string;
  userId?: string;
...
export async function handler(
  request: FastifyRequest<{ Body: UpsertContentPlanBody }>,
  reply: FastifyReply,
) {
  ...
  // Guest path
  if (effectiveGuestId) {
    const existing = await request.prisma.$queryRaw<StrategyContentPlanRow[]>`
      SELECT strategy_content_plan_id FROM strategy_content_plan
      WHERE deleted_at IS NULL AND guest_id = ${effectiveGuestId} LIMIT 1
    `;
    let created = false;
    let rowId: string;
    if (existing.length > 0) {
      rowId = existing[0].strategy_content_plan_id;
      await request.prisma.$executeRaw`
        UPDATE strategy_content_plan SET payload = ${payloadJson}, updated_at = NOW()
        WHERE strategy_content_plan_id = ${rowId}
      `;
    } else {
  ...
  // Legacy path
  let existing: StrategyContentPlanRow[];
  if (effectiveUserId !== null) {
    existing = await request.prisma.$queryRaw<StrategyContentPlanRow[]>`
      SELECT strategy_content_plan_id FROM strategy_content_plan
