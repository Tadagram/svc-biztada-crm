...
interface ConversionGatewayBody {
  guestId?: string;
  businessId?: string;
  userId?: string;
  payload: any;
...
export async function handler(
  request: FastifyRequest<{ Body: ConversionGatewayBody }>,
  reply: FastifyReply,
) {
  ...
  await request.prisma.$executeRaw`
    UPDATE strategy_conversion_gateway
    SET deleted_at = NOW()
  ...
  await request.prisma.$executeRaw`
    INSERT INTO strategy_conversion_gateway (strategy_conversion_gateway_id, business_id, user_id, guest_id, payload, is_demo, created_at, updated_at)
    VALUES (
      ${newId},
      ${effectiveBusinessId},
      ${effectiveUserId},
