...
export async function handler(
  request: FastifyRequest<{ Body: UpsertSeedingBody }>,
  reply: FastifyReply,
) {
  ...
  // Guest path
  if (effectiveGuestId) {
    const existing = await request.prisma.$queryRaw<StrategySeedingRow[]>`
      SELECT strategy_seeding_id FROM strategy_seeding
      WHERE deleted_at IS NULL AND guest_id = ${effectiveGuestId} LIMIT 1
    `;
    let created = false;
    let rowId: string;
    if (existing.length > 0) {
      rowId = existing[0].strategy_seeding_id;
      await request.prisma.$executeRaw`
        UPDATE strategy_seeding SET payload = ${payloadJson}, updated_at = NOW()
        WHERE strategy_seeding_id = ${rowId}
      `;
    } else {
  ...
  // Legacy path
  let existing: StrategySeedingRow[];
  if (effectiveUserId !== null) {
    existing = await request.prisma.$queryRaw<StrategySeedingRow[]>`
      SELECT strategy_seeding_id FROM strategy_seeding
