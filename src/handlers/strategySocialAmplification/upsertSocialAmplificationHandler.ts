...
interface SocialAmplificationBody {
  guestId?: string;
  businessId?: string;
  userId?: string;
  payload: any;
...
export async function handler(
  request: FastifyRequest<{ Body: SocialAmplificationBody }>,
  reply: FastifyReply,
) {
  ...
  await request.prisma.$executeRaw`
    UPDATE strategy_social_amplification
    SET deleted_at = NOW()
  ...
  await request.prisma.$executeRaw`
    INSERT INTO strategy_social_amplification (strategy_social_amplification_id, business_id, user_id, guest_id, payload, is_demo, created_at, updated_at)
    VALUES (
      ${newId},
      ${effectiveBusinessId},
      ${effectiveUserId},
